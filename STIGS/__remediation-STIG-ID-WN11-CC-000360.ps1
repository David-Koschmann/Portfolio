<#
.SYNOPSIS
    This PowerShell script disables Digest authentication on the WinRM client, since Digest auth is weaker than other options and may be subject to man-in-the-middle attacks.

.NOTES
    Author          : Davvid Koschmann
    LinkedIn        : www.linkedin.com/in/davidkoschmann
    GitHub          : https://github.com/David-Koschmann
    Date Created    : 19-AUG-2026
    Last Modified   : 19-AUG-2026
    Version         : 1.0
    CVEs            : N/A
    Plugin IDs      : N/A
    STIG-ID         : WN11-CC-000360
    Documentation   : https://stigaview.com/products/win11/v2r7/WN11-CC-000360/

.TESTED ON
    Date(s) Tested  : 19-AUG-2026
    Tested By       : David Koschmann
    Systems Tested  : Windows 11 Pro (Azure VM)
    PowerShell Ver. : 5.1

.USAGE
    Run as Administrator.
    PS C:\> .\__remediation-STIG-ID-WN11-CC-000360.ps1
#>

$RegPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client"

if (!(Test-Path $RegPath)) {
    New-Item -Path $RegPath -Force | Out-Null
}

New-ItemProperty -Path $RegPath -Name "AllowDigest" -Value 0 -PropertyType DWord -Force | Out-Null
